...
def inference(model, sess, dataset):
    ...
    with open(os.path.join(FLAGS.train_dir, 'result.txt'), 'w') as f:
        for label in result:
            f.write('%d\n' % label)
...
with tf.Session(config=config) as sess:
    if FLAGS.is_train:
        ...
        import os
        train_writer = SummaryWriter(os.path.join(FLAGS.train_dir, 'board_train'))
        val_writer = SummaryWriter(os.path.join(FLAGS.train_dir, 'board_val'))
