...
class TestCarve(unittest.TestCase):
    ...
    def test_remove_seam(self):
        for src in (self.gray, self.rgb):
            for _ in range(self.delta):
                gray = src if src.ndim == 2 else carve.rgb2gray(src)
                energy = carve.get_energy(gray)
                seam, _ = carve.get_backward_seam(energy)
                dst = carve.remove_seam(src, seam)
    ...
    def test_get_backward_seam(self):
        energy = carve.get_energy(self.gray)
        seam, cost = carve.get_backward_seam(energy)
    ...
    def test_get_forward_seam(self):
        out_seam, out_cost = carve.get_forward_seam(self.gray, None)
    ...
    def _naive_reduce_width(src, num_seams, energy_mode):
        for _ in range(num_seams):
            ...
            if energy_mode == 'backward':
                seam, _ = carve.get_backward_seam(carve.get_energy(gray))
            else:
                seam, _ = carve.get_forward_seam(gray, None)
            src = carve.remove_seam(src, seam)
        return src

    def test_reduce_width(self):
        for src in (self.gray, self.rgb):
            out = carve.reduce_width(src, self.delta, 'backward', None)
            ans = self._naive_reduce_width(src, self.delta, 'backward')
            assert (out == ans).all()

            out = carve.reduce_width(src, self.delta, 'forward', None)
    ...
    def _naive_expand_width(src, delta_width, energy_mode):
        gray = src if src.ndim == 2 else carve.rgb2gray(src)
        h, w = gray.shape
        seams_mask = carve.get_seams(gray, delta_width, energy_mode, None)
    ...
    def test_expand_width(self):
        out = carve.expand_width(self.gray, self.delta, 'backward', None)
        ans = self._naive_expand_width(self.gray, self.delta, 'backward')
        assert (out == ans).all()

    def test_resize_optimal(self):
        for src in (self.gray, self.rgb):
            with pytest.warns(UserWarning):
                dst = carve.resize(src, (self.lo_w, self.lo_h), 'forward',
                                   'optimal')
                assert dst.shape[:2] == (self.lo_h, self.lo_w)
            with pytest.warns(UserWarning):
                carve.resize(src, (self.lo_w, self.lo_h), 'backward', 'optimal',
                             self.gray)
                assert dst.shape[:2] == (self.lo_h, self.lo_w)
            dst = carve.resize(src, (self.hi_w, self.lo_h), 'backward',
                               'optimal')
            assert dst.shape[:2] == (self.lo_h, self.hi_w)
            dst = carve.resize(src, (self.hi_w, self.hi_h), 'backward',
                               'optimal')
    ...
    def test_resize(self):
        for src in (self.gray, self.rgb):
            ...
            assert (dst == src).all()
            dst = carve.resize(src, (self.w, self.h), 'backward', 'optimal')
            assert (dst == src).all()
