...
class malloc_buddy(malloc):
    def __init__(self, size, start, headerSize, align):
        super(malloc_buddy, self).__init__(size, start, headerSize, 'BEST', 'ADDRSORT', True, align)
        self.start = start
    ...
    def malloc(self, size):
        ...
        if bestIdx != -1:
            if bestSize > size:
                insertList = []
                currSize = size
                while currSize < bestSize:
                    insertList.append((bestAddr + currSize, currSize))
                    currSize *= 2
                self.freelist = self.freelist[:bestIdx] + insertList + self.freelist[bestIdx + 1:]
                self.addToMap(bestAddr, size)
            elif bestSize == size:
                self.freelist.pop(bestIdx)
                self.addToMap(bestAddr, size)
    ...
    def free(self, addr):
        ...
        while changed:
            ...
            for i in range(1, len(self.freelist)):
                eaddr, esize = self.freelist[i]
                if eaddr == (self.curr[0] + self.curr[1]) and self.curr[1] == esize and (self.curr[0] - self.start) % (2 * esize) == 0:
                    self.curr = (self.curr[0], self.curr[1] + esize)
                    changed = True
                else:
                    self.newlist.append(self.curr)
...
parser.add_option('-B', '--buddy',       default=False,      help='use buddy system?',                           action='store_true',           dest='buddy')

(options, args) = parser.parse_args()

m = None
if options.buddy:
    m = malloc_buddy(int(options.heapSize), int(options.baseAddr), int(options.headerSize), options.alignment)
else:
...
if options.opsList == '':
    ...
    for op in options.opsList.split(','):
        if op[0] == '+':
            ...
            if options.solve == True:
                print(' returned %d (searched %d elements)' % (ptr + options.headerSize, cnt), end='')
            else:
                print(' returned ?', end='')
            c += 1
        elif op[0] == '-':
            ...
            if index >= len(p):
                print('Invalid Free: Skipping', end='')
                continue
            print('Free(ptr[%d])' % index)
            rc = m.free(p[index])
            if options.solve == True:
                print('returned %d' % rc, end='')
            else:
                print('returned ?', end='')
        else:
            assert False, 'badly specified operand: must be +Size or -Index'
        if options.solve == True:
            m.dump()
        else:
            print('List?', end='')
