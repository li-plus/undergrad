...
class Disk:
    ...
    def GetNextIO(self):
        ...
        if self.policy == 'FIFO':
            ...
            next_block = -1
            next_index = -1
            for block, index in self.requestQueue:
                if self.requestState[index] == STATE_DONE:
                    continue
                interval = (block - self.currentBlock + len(self.blockToTrackMap)) % len(self.blockToTrackMap)
                if interval < min_interval:
                    min_interval = interval
                    next_block = block
                    next_index = index
            assert next_block != -1 and next_index != -1
            self.currentBlock, self.currentIndex = next_block, next_index
        else:
...
parser.add_option('-p', '--policy',          default='FIFO',      help='Scheduling policy (FIFO, SSTF, SATF, BSATF, CSCAN)',             action='store', type='string', dest='policy')
